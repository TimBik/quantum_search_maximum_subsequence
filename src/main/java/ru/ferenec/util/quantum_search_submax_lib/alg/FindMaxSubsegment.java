...
import ru.ferenec.util.quantum_search_submax_lib.util.ConvertRelevantToCorrect;
import ru.ferenec.util.quantum_search_submax_lib.util.Correct;
import ru.ferenec.util.quantum_search_submax_lib.util.Relevant;
...
public class FindMaxSubsegment {
    ...
    private int[] findCertainLengthSegment(Correct data, int d) {
        ...
        for (int i = 0; i < t; i++) {
            int randIndex = ThreadLocalRandom.current().nextInt(0, data.numberOfLastElement() + 1);
